...
- **Login uses a programmatic session, not the OTL flow.** `CacheTestBase::drupalLogin()` bypasses Drupal's one-time login (OTL) HTTP flow. On Pantheon, Cloudflare Bot Management intercepts automated HTTP clients (BrowserKit has no JavaScript execution) and returns a JS challenge page instead of forwarding the OTL request to PHP-FPM. The `SSESS` session cookie is never set, so BrowserKit's cookie jar stays empty and every `assertTrue(drupalUserIsLoggedIn())` call fails. The fix writes the session directly to the database (using Symfony's `_sf2_attributes` serialization, the same format Drupal uses) and injects the session cookie into BrowserKit's jar. The web server treats these sessions identically to ones produced by a normal login. Login is test infrastructure here — the assertions cover cache header behavior, not authentication. All cache-header assertions continue to go through Cloudflare via the anonymous Guzzle client (`$anonClient`) unchanged. This is the same approach Drupal core uses in `BrowserTestBase`, which seeds sessions programmatically rather than performing a real form-based login.
