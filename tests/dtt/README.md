...
- All invalidation tests follow the canonical `assertCacheMissOnSave($path, $entity)` sequence: warm cache → assert HIT → `$entity->save()` → assert MISS → assert HIT (re-cached).
...
3. **`$this->userA->save()` in `testDynamicCacheMissAfterAccountChange()`** — This is a direct entity API save in the test process. That invalidation is written to Redis by the test process and must be visible to the web server for the subsequent `assertDynamicCacheMiss()` to observe a MISS.
