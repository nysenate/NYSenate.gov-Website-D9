...
**Anonymous cache non-invalidation** (`AnonymousCacheNonInvalidationTest`) — **local DDEV only, excluded from Pantheon CI; see below**
...
**Why this class is excluded from the Pantheon CI run.** Cloudflare's BAN dispatch has no observable "propagation complete" signal — Pantheon's edge-key API confirms only that the BAN was *accepted*, not that every CF edge has processed it. That makes the race described above fundamentally a timing guess against an unbounded window, not something a fixed or adaptive wait can close with certainty. Three escalating fixes were tried directly against a Pantheon multidev: fixed sleeps between tests, a tolerant retry on the final assertion (rejected — a leftover-propagation MISS and a genuine one-off over-invalidation bug are indistinguishable from the outside, so tolerating one silently tolerates the other), and the settle-before-assert approach described above, which measurably reduced but did not eliminate the flake rate in practice. Given that over-invalidation (an extra cache rebuild) is a minor performance concern rather than a correctness bug — unlike failure-to-invalidate, which is asserted strictly and reliably in `CacheMissInvalidationTest` — the two `AnonymousCacheNonInvalidationTest` chunks are marked `ci: false` in `tests/dtt/test-chunks.yml` and skipped by `run-all-chunks.sh`. They remain fully enabled for local DDEV runs (`ddev run-cache-tests`), where there is no CDN layer and the race cannot occur, so this behavior still has real regression coverage — just not on every PR.
