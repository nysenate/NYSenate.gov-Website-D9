  ...
  Drupal.ImageWidgetCropType.prototype.setValue = function (name, value, delta) {
    ...
    if (delta && name !== 'applied') {
      ...
      if (value < this.hardLimit[name]) {
        let originValue = name === 'width' ? this.originalWidth : name === 'height' ? this.originalHeight : null;
        // If image is too small use image size, otherwise use hard limit size.
        value = Math.min(this.hardLimit[name], originValue);
      }
    }
