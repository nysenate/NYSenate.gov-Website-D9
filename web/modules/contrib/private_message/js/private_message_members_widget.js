...
(($, Drupal, drupalSettings, window, once) => {
  ...
  let container;
  let inputWrapper;
  let autocompleteUsernames;
  let autocompleteResultsContainer;
  let insertedUsernames;
  ...
  function insertUsernameOnComplete(inputCount, username) {
    ...
    const inputs = inputWrapper.find('input[type="text"]');
  ...
  function addUserToMembers(username, validateName) {
    ...
    if (username.length && !insertedUsernames[username]) {
      ...
      // reference widget.
      inputWrapper.find('input[type="text"]').each(() => {
      ...
      if (!found) {
        insertUsernameOnComplete(
          inputWrapper.find('input[type="text"]').length,
          username,
        );
      ...
      if (maxMembers && Object.keys(insertedUsernames).length >= maxMembers) {
        usernameInput
          .parents('form:first')
          .find(
            '.private_message_message_widget_default_wrapper:first textarea:first',
          )
          .focus();
  ...
  function markUsernameInvalid(username) {
    container.find('.private-message-member-display-item').each(() => {
  ...
  function showAutocompleteResults(string) {
    ...
    if (
      ...
      autocompleteUsernames[string] &&
      autocompleteUsernames[string].length
    ) {
      let i;

      // The position of the input in the DOM. The results are inserted into the
      // DOM relative to this input.
      const position = usernameInput.position();
      // Create the results container.
      autocompleteResultsContainer = $('<div/>', {
        id: 'pm-members-autocomplete-results-wrapper',
      }).css({
        top: `${position.top + usernameInput.outerHeight() + 2}px`,
      ...
      const list = $('<ul/>', {
        class: 'ui-front ui-menu ui-widget ui-widget-content',
      });

      // Create each of the list elements.
      i = 1;
      $.each(autocompleteUsernames[string], (index) => {
        const { username } = autocompleteUsernames[string][index];
        const { uid } = autocompleteUsernames[string][index];
        if (!insertedUsernames[username]) {
          $('<li/>', { class: 'ui-menu-item' })
            .append(
              $('<a/>', {
                class: 'pm-autocomplete-search-result',
    ...
    once(
      ...
      $(item)
        .keydown((e) => {
          ...
          if (keyCode === 9) {
            if (usernameInput.val().length) {
              ...
              // Check if provided input text is a comma seperated list of users.
              const multipleUsernames = usernameValue.split(',');
              if (multipleUsernames.length > 1) {
                // Iterate each username
                multipleUsernames.forEach((username) => {
                  addUserToMembers(username, true);
                });
              } else {
                addUserToMembers(usernameValue, true);
              }
              usernameInput.val('');
              hideAutocompleteResults();
            }
          ...
          // deleted.
          else if (keyCode === 8 && !$(item).val().length) {
            $(item)
              .parent()
              .children('.private-message-member-display-item:last')
              .children('.pm-username-remove-link:last')
              .click();
          }
        })
        // Act on keyup.
            ...
            timeout = window.setTimeout(() => {
              ...
              if (username.length) {
                if (!autocompleteUsernames[username]) {
                  $.ajax({
                    url: drupalSettings.privateMessageMembersWidget
                      .callbackPath,
                    data: { username },
                    success(data) {
                      triggerCommands(data);
        ...
        .blur(() => {
          ...
          usernameInputTimeout = window.setTimeout(() => {
            if (usernameInput.val().length) {
              const usernameValue = usernameInput.val();
              // Check if provided input text is a comma seperated list of users.
              const multipleUsernames = usernameValue.split(',');
  ...
  function autocompleteListener() {
    ...
    if (autocompleteResultsContainer) {
      once(
        'pm-autocomplete-listener',
        'ul li a',
        autocompleteResultsContainer[0],
      ).forEach((item) => {
        // If a search result is clicked, the username is added to the list of
        // members.
        $(item)
          .click(addUserToMembersClickHandler)
  ...
  function removeMember(element) {
    const username = element
      .parent()
      .children('.pm-username:first')
      .attr('data-pm-username');
  ...
  function membersWatcher(context) {
    $(
      once(
        'private-message-members-watcher',
        '.private-message-member-display-item .pm-username-remove-link',
        context,
      ),
    ).each(() => {
      $(this).click(() => {
  ...
  function init() {
    ...
    if (!initialized) {
      ...
      usernameInput = $('<input/>', {
        type: 'text',
        id: 'thread-members-input',
        placeholder: drupalSettings.privateMessageMembersWidget.placeholder,
      })
        .attr('autocomplete', 'off')
        .attr('size', drupalSettings.privateMessageMembersWidget.fieldSize);
      // Store the maximum number of members allowed to be added to the thread.
      maxMembers = Number(
        drupalSettings.privateMessageMembersWidget.maxMembers,
      );
      // The label for the field.
      const label = $('<label/>', { for: 'thread-members-input' }).text(
        Drupal.t('To:'),
      );
      ...
      // overrides.
      inputWrapper = $(
        '.private_message_members_widget_default_wrapper:first',
      ).hide();

      // Store the container to make for faster searches of the DOM in the
      // script.
      container = $('<div/>', { id: 'thread-members-display-container' })
        .append(label)
  ...
  Drupal.behaviors.privateMessageMembersWidget = {
    attach(context) {
      ...
      Drupal.AjaxCommands.prototype.privateMessageMembersAutocompleteResponse =
        (ajax, response) => {
      ...
      Drupal.AjaxCommands.prototype.privateMessageMemberUsernameValidated = (
        ajax,
        response,
      ) => {
    ...
    detach(context) {
      $(context)
        .find('.private-message-member-display-item .pm-username-remove-link')
        .unbind('click', removeMember);
      $(context)
        .find('.pm-autocomplete-search-result')
        .unbind('click', addUserToMembersClickHandler)
        .unbind('keydown', autompleteResultsKeydownNavigationHandler)
        .unbind('focus', autocompleteResultsFocusHandler);
    },
  };

  // Theme function to create the visual representation for users showing that a
  // user has been added to the members list.
  Drupal.theme.usernameDisplayItem = (userName) => {
    return $('<div />', {
      class: 'private-message-member-display-item',
      'data-username': userName,
    })
      .append(
        $('<span/>', {
          class: 'pm-username',
          'data-pm-username': userName,
        }).text(extractUsernameFromAutocompleteValue(userName)),
      )
      .append($('<span/>', { class: 'pm-username-remove-link' }).text('X'));
  };
