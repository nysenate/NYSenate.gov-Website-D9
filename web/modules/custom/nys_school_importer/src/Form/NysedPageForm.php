...
class NysedPageForm extends FormBase {
   ...
   * {@inheritdoc}
   */
  public function __construct(ImporterHelper $importer_helper, State $state, ExtensionPathResolver $path_resolver, Messenger $messenger, FileSystem $file_system) {
  ...
  public function importFinished($success, $results, $operations) {
    ...
    if ($nys_school_nysed_importer_failed == TRUE) {
      ...
      $response = new RedirectResponse('admin/nys-school-report');
      $response->send();
