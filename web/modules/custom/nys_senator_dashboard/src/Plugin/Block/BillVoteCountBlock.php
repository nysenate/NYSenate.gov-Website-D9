...
class BillVoteCountBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $active_senator_district_id = $this->managedSenatorsHandler
      ->getActiveSenatorDistrictId($this->currentUser->id());
    $yes_count = 0;
    $no_count = 0;
    ...
    $props = [
      ...
      'yes_count' => $yes_count,
      'no_count' => $no_count,
      'yes_percentage' => $yes_count + $no_count > 0 ? round(($yes_count / ($yes_count + $no_count)) * 100, 2) : 0,
      'no_percentage' => $yes_count + $no_count > 0 ? round(($no_count / ($yes_count + $no_count)) * 100, 2) : 0,
      'in_district_total_count' => $in_district_yes_count + $in_district_no_count,
      'in_district_yes_count' => $in_district_yes_count,
      'in_district_yes_percentage' => ($in_district_yes_count + $in_district_no_count) > 0 ? round(($in_district_yes_count / ($in_district_yes_count + $in_district_no_count)) * 100, 2) : 0,
      'in_district_no_percentage' => ($in_district_yes_count + $in_district_no_count) > 0 ? round(($in_district_no_count / ($in_district_yes_count + $in_district_no_count)) * 100, 2) : 0,
      ...
      'out_district_yes_percentage' => ($yes_count + $no_count - ($in_district_yes_count + $in_district_no_count)) > 0
        ? round((($yes_count - $in_district_yes_count) / ($yes_count + $no_count - ($in_district_yes_count + $in_district_no_count))) * 100, 2)
        : 0,
      'out_district_no_percentage' => ($yes_count + $no_count - ($in_district_yes_count + $in_district_no_count)) > 0
        ? round((($no_count - $in_district_no_count) / ($yes_count + $no_count - ($in_district_yes_count + $in_district_no_count))) * 100, 2)
        : 0,
    ];
    $ret['#props'] = $props;
